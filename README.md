...
- Standard front, back, left, right, top, and perspective camera views with one adjustable directional section cut
- Named views stored in the project file with camera, semantic layer, and section state
...
The construction domain under `src/domain/` does not depend on React or Three.js. Project files store authored inputs and portable named-view definitions; generated meshes and material totals are rebuilt on load. Named views reference semantic systems, scopes, and roles rather than generated member IDs. Generated parts carry semantic system, scope, role, and kind classifications that drive viewport visibility without changing the design or estimate. Fastener quantities are purchasing allowances derived from modeled connections and installation patterns, not an engineered fastening, uplift, or site-anchoring schedule.
